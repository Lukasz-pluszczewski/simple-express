...
export type Config = {
  ...
  cookieParser?:
    | [secret?: string | string[], options?: CookieParseOptions]
    | false;
...
> = {
  ...
  run: <TParams extends unknown[]>(
    fn: (...params: TParams) => Promise<void>,
    ...params: TParams
  ) => Promise<void>;
};

export type GetContextContainerHelper<
  TContext extends Record<string, unknown> = Record<string, never>,
> = () => {
  get: <TKey extends keyof TContext>(key: TKey) => TContext[TKey];
  set: <TKey extends keyof TContext>(key: TKey, value: TContext[TKey]) => void;
...
> = {
  ...
  res: Response;
  requestContext: TRequestContext extends false
    ? TRequestContext extends false
      ? never
      : never
    : ContextContainer<TRequestContext>;
  globalContext: TGlobalContext extends false
...
export type SingleHandler<
  ...
  TGlobalContext extends Record<string, unknown> = Record<string, never>,
> = (
  handlerParams: HandlerParams<TLocals, TRequestContext, TGlobalContext>
...
export type GetHandlerParams = <
  ...
  TGlobalContext extends Record<string, unknown> = Record<string, never>,
>(
  params: HandlerParams<TLocals, TRequestContext, TGlobalContext>
    & AdditionalRouteParams
) => Record<string, any>;
export type GetErrorHandlerParams = <
  AdditionalRouteParams extends Record<string, unknown> = Record<string, never>,
  TLocals extends Record<string, unknown> = Record<string, never>,
  TRequestContext extends Record<string, unknown> = Record<string, never>,
  TGlobalContext extends Record<string, unknown> = Record<string, never>,
...
export type MapResponse = <
  AdditionalRouteParams extends Record<string, unknown> = Record<string, never>,
>(
  responseObject: ResponseDefinition,
...
export type Plugin<
  ...
  TGlobalContext extends Record<string, unknown> = Record<string, never>,
> = (
  config: SimpleExpressConfigForPlugins<
    AdditionalRouteParams,
    TLocals,
    TRequestContext,
    TGlobalContext
...
export type GlobalContextConfig<
