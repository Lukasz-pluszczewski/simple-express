...
export const createHandler =
  <
    AdditionalRouteParams extends Record<string, unknown> = Record<
      string,
      never
    >,
  ...
  >({
    ...
    plugins: ReturnType<
      Plugin<AdditionalRouteParams, TLocals, TRequestContext, TGlobalContext>
    >[];
    requestContextConfig?:
      | RequestContextConfig<TRequestContext, AdditionalRouteParams, TLocals>
      | false;
  ...
  async (req: RequestObject, res: Response, next: NextFunction) => {
    ...
    const requestContextContainer =
      requestContextConfig === false || requestContextConfig === undefined
        ? undefined
    ...
    } as HandlerParams<TLocals, TRequestContext, TGlobalContext>
      & AdditionalRouteParams);
...
export const createErrorHandler =
  ...
  async (error: Error, req: Request, res: Response, next: NextFunction) => {
    ...
    ) as TRequestContext extends false
      ? TRequestContext extends false
        ? never
        : never
      : ContextContainer<TRequestContext>;

    const handlerParams = await chainPlugins<
      Omit<HandlerParams<TLocals, TRequestContext, TGlobalContext>, 'params'>
        & AdditionalRouteParams,
      any
    >(
      plugins,
      ...
      ...contextParams,
      requestContext: requestContextContainer,
      globalContext: globalContextContainer as TGlobalContext extends false
        ? TGlobalContext extends false
          ? never
          : never
        : ContextContainer<TGlobalContext>,
    } as Omit<HandlerParams<TLocals, TRequestContext, TGlobalContext>, 'params'>
      & AdditionalRouteParams);
    ...
    const mappedResult = await chainPlugins(
      plugins,
      'mapResponse',
      (previousResult: ResponseDefinition) =>
        !previousResult || previousResult.type === 'none'
    )(result, handlerParams);
    sendResponse(req, res, mappedResult as any);
  };
